...
RMB + mouse looks around; WASD flies, Q/E moves vertically, Shift accelerates. Keys 1–6 select Earth, Jupiter, Moon, Mars, dawn and belt views. T starts/stops the tour, O orbits, F returns to free flight. Space pauses the system; +/- changes exposure, X toggles adaptation. F1 toggles help, F2 quality, F3 the belt transmittance map, F4 the belt extinction, F5 toggles temporal anti-aliasing, F9 cycles the spatial pass (off, FXAA, SMAA), Alt+Enter toggles borderless fullscreen, F6 cycles the rock splat cut-off (off, 1.2, 2.5, 4 px), F7 lights the splats in both culling passes instead of the scatter pass only, F8 cycles the tone curve (Khronos PBR Neutral by default, AgX, ACES filmic), F10 saves a capture, F11 toggles the belt dust, F12 shows the control panel (`--ui` opens it at start), Esc exits.
