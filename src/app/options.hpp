...
inline constexpr std::string_view usage =
    ...
    "0..5 (shaded, tile coordinate, normal, elevation, crater shadow, morph and level) "
    "--terrain-trace deg (log why any patch drawn that far around the body is there) "
    "--tier-activate px (lower switches to the patches further out, "
    ...
    "capture=mars.png, a multi-value option taking commas as in sun-at=1.2,0.4, name=x labelling the shot; the "
    "command line sets the defaults) --report file.json (per-shot readings) --headless "
    "(hidden window) --ui (control panel open at start) --help\n"
    "Controls: RMB mouse look; MMB hold telescope (5x); WASD move; Q/E vertical; Shift fast; 1-9 bookmarks (the "
    "tenth, the minor planet up close, is --bookmark 9 or the panel); O orbit; F free;\n"
...
struct Options {
    ...
    float terrain_trace = render::TerrainSettings{}.trace_arc;       // log patches drawn this far around the body
