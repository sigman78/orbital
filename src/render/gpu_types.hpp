...
static_assert(offsetof(CullParams, index_counts) == 272 && sizeof(CullParams) == 272 + 4 * ORBITAL_ROCK_GROUPS);
static_assert(offsetof(CullScratch, instances) == sizeof(CullParams) &&
              offsetof(CullScratch, counts) == sizeof(CullParams) + 8);
static_assert(offsetof(CullScratch, args) == sizeof(CullParams) + 8 + 8 * (ORBITAL_ROCK_GROUPS + 1));
static_assert(offsetof(FrameData, camera_time) == 64 && sizeof(FrameData) == 528);
