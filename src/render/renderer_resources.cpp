...
void Renderer::Impl::create_device(void* window) {
    ...
    buffers.belt_state = UniqueGpuHeap::create(device, 2 * belt_slot_bytes(), gpu::MemoryType::gpu_only);
    buffers.belt_state_staging = UniqueGpuHeap::create(device, 2 * belt_slot_bytes()); // two slots, by frame parity
    belt_candidates.resize(belt_capacity);
