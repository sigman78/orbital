...
$assets = @(
    ...
    @{ Name = 'gas_albedo';     Source = '8k_jupiter.jpg';        Url = $solar; Hash = '0BD844BF20822C4E3E80882B077859833C0DAC44C7E4E1E0CD63D1B1B6D43085'; License = 'CC-BY-4.0'; Attribution = 'Solar System Scope'; ColorSpace = 'sRGB' }
...
function Convert-Texture($sourcePath, $outputPath) {
    $source = [System.Drawing.Image]::FromFile($sourcePath)
    try {
        $sourceWidth = $source.Width
        $sourceHeight = $source.Height
        if ($sourceWidth -le $MaxWidth) {
        ...
        $width = $MaxWidth
        $height = [int][Math]::Round($sourceHeight * $MaxWidth / $sourceWidth)
...
foreach ($asset in $assets) {
    ...
    $dimensions = Convert-Texture $sourcePath $outputPath
