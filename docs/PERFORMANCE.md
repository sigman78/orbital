...
The saving is in every pass in proportion (the atmosphere group -8 to -10 percent, the surface and post groups likewise) and not in the present scope, so it is not our presentation cost: a hidden window is not composed by the desktop each frame, and the GPU stops sharing its time with the compositor. Hidden numbers are therefore cleaner but not what a viewer's frame costs, and they cannot be compared with any recorded baseline. The suite records `headless` in the protocol only when set, so every existing baseline stays comparable with visible runs and a hidden run refuses a visible baseline. Benchmarks stay on the visible window; the hidden mode is for captures and checks, where timing is not the point. A hidden baseline series can be started if the quieter numbers are ever preferred, and would then stand on its own.
