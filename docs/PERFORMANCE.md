...
Close other demo instances and GPU-heavy applications. The suite first runs `tools/check-bar1.py`, which reads the host-visible aperture (BAR1) from `nvidia-smi`, prints it, and warns under 150 MiB free: a full aperture puts the mapped heaps in system memory and the numbers describe the machine, not the change (see ARCHITECTURE.md on the heaps). The run goes ahead; the state is recorded in the report and flagged at the top of its summary. Use the same display, power mode and build configuration. Do not rebuild or edit runtime shaders/assets during a run. The script records GPU/driver details (driver query when `nvidia-smi` is available), OS/CPU, actual drawable dimensions, settings, Git revision/dirty status and executable/shader/asset hashes. Dirty-source baselines are labeled as such; binary hashes identify the measured artifact, and a source revision alone does not prove which executable was built. Cached asset changes require a new compatible baseline. Defaults not exposed by CLI, including quality internals, remain part of the versioned implementation under test.
...
## Confirming run for the flagged fullscreen belt increases (2026-09-15)
...
## Atmosphere pass bounded to the projected shell (2026-09-15)

The first change measured on the hidden series: [atmosphere-bounds](performance/atmosphere-bounds.md) ([JSON](performance/atmosphere-bounds.json)) against `headless-baseline`, full protocol. GPU frame at fullscreen: earth 8.01 to 7.80 ms, dawn 7.31 to 7.02, belt-sun 7.17 to 6.89, giant 11.03 to 10.89; every windowed view within 0.03 ms; the belt view unchanged (its giant's box is the frame). The atmospheres child in the views with a body at mid size drops 9 to 14 percent, in far views by half or more. Step tiers by projected size on top of the bounds were tried and dropped (no measurable gain, a visible shift at 120 px of radius; DECISIONS.md).
