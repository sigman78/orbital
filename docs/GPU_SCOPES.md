...
The frame scope records total time and resets per-frame recording status. Each pass has its own two timestamp slots; enum order need not match recording order. The enum lives in `render/gpu_pass.hpp` with a table giving each pass its benchmark column, panel label and parent group; `FrameStats::gpu` is the array of their milliseconds, the benchmark writes one column per row of the table and the panel derives each group's children from it, so a new scope needs a row there and nothing else. Nested scopes cover the cull/shadow aggregate and individual sections. Scopes are noncopyable and nonmovable, close on early returns, and only record timestamps: they never submit or wait. Debug assertions reject duplicate pass IDs, use outside a timing frame, and closing a frame while a pass remains open.
