...
- Belt self-shadowing uses one orthographic depth map over the whole belt rather than a camera-following one: the annulus is thin and the sun sits about 26 degrees above its plane, so it fits a light-space rectangle (outer radius wide, outer times cos(tilt) tall) that a 4096 map resolves at about 0.03 units per texel from any viewpoint without popping. Only rocks of radius 0.05 and above cast (the larger size-tail rocks, about 3% of the belt); they are culled by a second run of the compute pass with an orthographic box test into their own instance list and drawn with the same indirect multi-draw. Belt rocks receive from this map, bodies keep the planet-anchored map. Measured at the frozen belt view, 1080p, 520k rocks: cull plus shadow rises from 0.25 to 0.66 ms (1.02 ms with casters from radius 0.03, 0.61 ms with 9x rocks only, 0.71 ms at 2048 texels). The belt's shadow on the gas giant was strengthened 30 percent at the same time.
