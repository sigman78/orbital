...
- GPU-driven belt culling replaces the CPU cluster pass: a compute shader runs one thread per rock (frustum with jitter padding, analytic occlusion by every body, projected-size level pick, billboard demotion), then a count, prefix and scatter sequence writes per-group instance lists and indirect draw arguments that the CPU submits as one indirect draw per group. Everything goes through buffer addresses because the conventional descriptor set has no storage buffers; the fork gained conventional-backend paths for compute pipelines and indirect draws. Depth-pyramid occlusion was not added: the only occluders that matter here are the bodies, which the analytic test covers, and the conventional path has no storage images to build a pyramid with. Measured at the frozen belt view, 1080p, GTX 1080 Ti (`--rocks` overrides the belt size for such runs): at 65k rocks the CPU pass took 1.25 ms per frame and the GPU pass 0.21 ms of CPU with about 1.1 ms more GPU time (fixed cost of the three dispatches and 97 indirect draws); at 650k rocks the CPU pass took 13.5 ms and the frame 18.9 ms, the GPU pass 0.23 ms of CPU and a 6.9 ms frame. The benchmark CSV records the CPU preparation time per frame separately from the vsync-bound frame time.
