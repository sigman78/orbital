...
- Set 0, binding 0 is a fixed array of `ORBITAL_TEXTURE_COUNT` (56) separate sampled images. The fork's conventional set layout declares the same count (`conventional_texture_descriptor_count` in `NoGraphicsAPI.cpp`) and `tools/check-shaders.py` holds the two equal: a descriptor written past the layout's array is a validation error and a crash, so a new slot changes both, and the compat patch with them.
- Set 0, binding 1 is a fixed array of 4 separate samplers.
- Set 0, binding 2 is a fixed array of `ORBITAL_TEXTURE_ARRAY_COUNT` (8) separate sampled images for `Texture2DArray` textures. The fork declares the matching count (`conventional_texture_array_descriptor_count`), and `check-shaders.py` holds them equal. The renderer binds a placeholder to every array slot at start-up. `write_texture_array_descriptor` writes the binding by slot index.
