...
The Slang shaders implement this ABI in `shaders/common.slang` and `shaders/scene_shared.h`. Reflection verifies texture binding 0/count 45 (`ORBITAL_TEXTURE_COUNT`), sampler binding 1/count 4, and a 32-byte push root per pipeline: three typed 64-bit GPU pointers plus `base` and `mode` for the surface and fullscreen shaders (the surface pipelines share one vertex stage, `surface.slang`, and have one fragment shader per body kind over `surface_common.slang`; the kind and mode numbers are defined once in `scene_shared.h`), the rock data and scratch pointers for the culling compute shader, and a vertex pointer with a pixel scale for the overlay. Compile with SPIR-V 1.6, column-major matrix layout and `-fvk-use-entrypoint-name`.
